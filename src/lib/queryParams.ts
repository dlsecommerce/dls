...
/**
 * Genérico: recebe as colunas nas quais buscar (ex: ["Nome", "Marca", "Referência"])
 */
export function buildOrSearchParts(tokens: string[], columns: string[]) {
  ...
  for (const term of tokens) {
    ...
    for (const v of variants) {
      ...
      for (const column of columns) {
        orParts.push(`${column}.ilike.${pattern}`);
      }
    }
  }
