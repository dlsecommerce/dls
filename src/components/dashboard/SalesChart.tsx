import React from 'react';
import {
  AreaChart,
  Area,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  ResponsiveContainer
} from 'recharts';
import { GlassmorphicCard } from "@/components/ui/glassmorphic-card";
import { motion } from 'framer-motion';
import { TrendingUp } from 'lucide-react';

const data = [
  { mes: 'Jan', vendas: 4500 },
  { mes: 'Fev', vendas: 5200 },
  { mes: 'Mar', vendas: 4800 },
  { mes: 'Abr', vendas: 6100 },
  { mes: 'Mai', vendas: 7200 },
  { mes: 'Jun', vendas: 6800 },
  { mes: 'Jul', vendas: 8500 }
];

export default function SalesChart() {
  const totalVendas = data.reduce((sum, item) => sum + item.vendas, 0);
  const mediaVendas = totalVendas / data.length;
  const crescimento = (
    ((data[data.length - 1].vendas - data[0].vendas) / data[0].vendas) * 100
  ).toFixed(1);

  return (
    <GlassmorphicCard className="p-4 sm:p-6">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.3 }}
      >
        {/* Header */}
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-4 sm:mb-6">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1 sm:mb-2">
              <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-[#2699fe] flex-shrink-0" />
              <h3 className="text-base sm:text-xl font-bold text-white">
                Vendas
              </h3>
            </div>
            <p className="text-xs sm:text-sm text-neutral-400">
              Últimos 7 meses
            </p>
          </div>

          <div className="min-w-0 sm:text-right">
            <p className="text-2xl sm:text-3xl font-bold text-white leading-tight break-words">
              R$ {totalVendas.toLocaleString()}
            </p>
            <div className="flex items-center gap-2 sm:justify-end mt-1 flex-wrap">
              <span className="text-xs sm:text-sm text-green-400 font-semibold">
                ↑ {crescimento}%
              </span>
              <span className="text-[11px] sm:text-xs text-neutral-500">
                vs período anterior
              </span>
            </div>
          </div>
        </div>

        {/* Chart */}
        <div className="w-full h-[220px] sm:h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart
              data={data}
              margin={{ top: 10, right: 8, left: -20, bottom: 0 }}
            >
              <defs>
                <linearGradient id="colorVendas" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#2699fe" stopOpacity={0.3} />
                  <stop offset="95%" stopColor="#2699fe" stopOpacity={0} />
                </linearGradient>
              </defs>

              <CartesianGrid
                strokeDasharray="3 3"
                stroke="rgba(255,255,255,0.05)"
              />

              <XAxis
                dataKey="mes"
                stroke="#666"
                tick={{ fontSize: 11 }}
                axisLine={false}
                tickLine={false}
              />

              <YAxis
                stroke="#666"
                tick={{ fontSize: 11 }}
                axisLine={false}
                tickLine={false}
                width={42}
                className="hidden sm:block"
              />

              <Tooltip
                contentStyle={{
                  backgroundColor: 'rgba(17, 17, 17, 0.95)',
                  border: '1px solid rgba(255,255,255,0.1)',
                  borderRadius: '12px',
                  backdropFilter: 'blur(10px)',
                  color: '#fff',
                  fontSize: '12px'
                }}
                formatter={(value) => [`R$ ${value}`, 'Vendas']}
              />

              <Area
                type="monotone"
                dataKey="vendas"
                stroke="#2699fe"
                strokeWidth={2.5}
                fillOpacity={1}
                fill="url(#colorVendas)"
                animationDuration={1000}
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-white/10">
          <div className="rounded-xl bg-white/[0.02] sm:bg-transparent p-3 sm:p-0">
            <p className="text-[11px] sm:text-xs text-neutral-400 mb-1">
              Total
            </p>
            <p className="text-base sm:text-lg font-bold text-white break-words">
              R$ {totalVendas.toLocaleString()}
            </p>
          </div>

          <div className="rounded-xl bg-white/[0.02] sm:bg-transparent p-3 sm:p-0">
            <p className="text-[11px] sm:text-xs text-neutral-400 mb-1">
              Média Mensal
            </p>
            <p className="text-base sm:text-lg font-bold text-white break-words">
              R$ {mediaVendas.toFixed(0)}
            </p>
          </div>

          <div className="rounded-xl bg-white/[0.02] sm:bg-transparent p-3 sm:p-0">
            <p className="text-[11px] sm:text-xs text-neutral-400 mb-1">
              Crescimento
            </p>
            <p className="text-base sm:text-lg font-bold text-green-400">
              +{crescimento}%
            </p>
          </div>
        </div>
      </motion.div>
    </GlassmorphicCard>
  );
}