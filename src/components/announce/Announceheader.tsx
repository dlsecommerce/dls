...
// ---------------------------------------------------------------------
export default function AnnounceTableHeaderBar({
  allSelected,
  hasRows,
  situacao,
  sortColumn,
  ...
  // O botão de restaurar só faz sentido no filtro "Excluídos"
  // (é o único contexto em que os itens selecionados têm deleted_at != null)
  const showRestoreButton = situacao === "Excluídos";

  // -----------------------------------------------------------------
  // Fecha o menu de situação ao clicar fora
  // -----------------------------------------------------------------
  useEffect(() => {
  ...
  return (
    <TableHeader className="[&_tr]:border-neutral-900">
      {/* ================================================================
          LINHA 1 — ações em massa (selecionar tudo / limpar / restaurar / excluir)
      ================================================================= */}
          ...
          <div className="flex items-center justify-center gap-2">
            {hasRows && (
              <button
                ...
                title="Selecionar todos os anúncios filtrados"
            ...
            {showRestoreButton && (
              <button
                type="button"
                onClick={hasSelectedItems ? onRestoreSelected : undefined}
                disabled={!hasSelectedItems}
                aria-label={
                  hasSelectedItems
                    ? `Restaurar ${selectedCount} selecionado(s)`
                ...
                className={`flex h-8 w-8 items-center justify-center border transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-emerald-500 ${
                  hasSelectedItems
                    ? "cursor-pointer border-emerald-500/30 text-emerald-500 hover:border-emerald-500/50 hover:bg-emerald-500/10"
            ...
            <button
              ...
              disabled={!hasSelectedItems}
              aria-label={
                hasSelectedItems ? `Excluir ${selectedCount} selecionado(s)` : "Selecione itens para excluir"
              }
              title={
                hasSelectedItems
                  ? selectedCount === 1
