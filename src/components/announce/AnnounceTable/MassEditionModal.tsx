...
export default function MassEditionModal({
  ...
  const handleDownloadTemplate = async () => {
    try {
      const downloadedFilename = await baixarModeloPlanilha();

      await createNotification({
        title: "Modelo de planilha baixado",
        message: `O modelo "${downloadedFilename}" foi baixado para edição em massa de anúncios.`,
        action: "comment",
        entityType: "announcement",
      });
    } catch (error) {
      console.error("Erro ao baixar modelo da planilha:", error);
    }
  };
