  ...
  const handleSave = useCallback(
    async (produto: any, composicao: any[], onAfterSave?: () => void) => {
      ...
      try {
        ...
        // =====================================================
        // REGRA IMPORTANTE:
        // - UPDATE: não limpar todos os campos 1..10, pois isso pode apagar
        //   composição válida e fazer o custo voltar para 0.
        // - INSERT: pode preencher os 10 campos com null por ser novo cadastro.
        const camposComposicao: Record<string, any> = {};

        if (!idAtualStr) {
          // Novo cadastro: inicia todos os campos
          for (let i = 1; i <= 10; i++) {
        ...
        composicao?.slice?.(0, 10)?.forEach?.((c: any, i: number) => {
          ...
          // ✅ Só envia código se realmente existir valor
          if (codigo) {
            camposComposicao[`Código ${idx}`] = codigo;
          } else if (!idAtualStr) {
            // Só manda null em cadastro novo
            camposComposicao[`Código ${idx}`] = null;
          }

          // ✅ Só envia quantidade se for válida e maior que zero
