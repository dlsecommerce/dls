...
function getMagaluNotificationLink(params: {
  ...
  return `/dashboard/marketplaces/magalu/edit?id=${encodeURIComponent(
...
function marcarTabelaMagaluParaRecarregar(detail?: any) {
  if (typeof window === "undefined") return;

  try {
    const value = String(Date.now());

    sessionStorage.setItem("magalu-pricing-precisa-recarregar", value);
...
export function useMarketplaceDetails(
  ...
  const buscarMarketplacePorIdInterno = async (
    ...
    for (const campo of campos) {
      ...
      if (error) {
        console.error("Erro ao buscar marketplace Magalu:", {
          tabela,
          campo,
          idBusca,
          error,
    ...
    console.warn("Marketplace Magalu não encontrado:", {
      tabela,
      idBusca,
      campos,
    });
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      if (!idInternoAnuncio) {
        console.warn("Marketplace Magalu sem ID interno do anúncio:", {
          idParam,
          lojaParam,
          lojaCode,
          mp,
  ...
  const save = useCallback(async () => {
    ...
    const tabela =
      lojaCode === "SB" ? "marketplace_magalu_sb" : "marketplace_magalu_pk";
    ...
    try {
      ...
      for (const variacao of variacoes) {
        ...
        if (
          ...
          console.warn("Variação Magalu sem identificador para salvar:", {
            variacao,
          });
      ...
      await createNotification({
        ...
        message: `A precificação Magalu do anúncio "${getProdutoLabel(
          produto,
        )}" foi atualizada.`,
        action: "update",
        entityType: "marketplace_magalu_pricing",
        entityId: String(produto.id_logico || produto.anuncio_id),
        link: getMagaluNotificationLink({
          marketplaceId: String(produto.id_logico || produto.anuncio_id),
          loja: getLojaLabel(lojaCode),
        }),
      });

      marcarTabelaMagaluParaRecarregar({
      ...
      console.error("Erro ao salvar precificação Magalu:", error);
