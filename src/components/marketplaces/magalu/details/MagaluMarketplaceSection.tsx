...
type MagaluMarketplaceSectionProps = {
  marketplaces: MagaluMarketplaceItem[];
  setMarketplaces: any;
  loading?: boolean;
...
// Helpers
// ================================
const getRowTitle = (item: MagaluMarketplaceItem) => {
...
const getRowDescription = (item: MagaluMarketplaceItem) => {
  const id = String(item?.id_anuncio || "").trim();
  const sku = String(item?.sku || "").trim();

  if (id && sku) return `ID Magalu ${id} · SKU ${sku}`;
  if (id) return `ID Magalu ${id}`;
  if (sku) return `SKU ${sku}`;

  return "Sem identificação Magalu";
};
...
// Linha Magalu
// ================================
type MagaluMarketplaceItemRowProps = {
  item: MagaluMarketplaceItem;
  idx: number;
  marketplaces: MagaluMarketplaceItem[];
  setMarketplaces: any;
...
const MagaluMarketplaceItemRow: React.FC<MagaluMarketplaceItemRowProps> = ({
  item,
  idx,
  marketplaces,
  setMarketplaces,
  ...
  const updateField = (key: keyof MagaluMarketplaceItem, value: string) => {
    if (disabled) return;
    ...
    <div className="group grid grid-cols-1 items-end gap-2 border-b border-white/10 py-2 last:border-b-0 sm:grid-cols-[minmax(0,1fr)_130px_130px_auto] sm:gap-2.5">
        ...
        <div className="mt-2">
          <div className="relative">
            ...
            <Input
              type="text"
              inputMode="url"
              placeholder="Link do anúncio Magalu"
              value={item.link || ""}
              disabled={disabled}
              title={disabled ? "Campo bloqueado para edição" : undefined}
      ...
      <Button
        ...
        className="
          ...
          sm:h-10 sm:w-10
        "
        title={disabled ? "Remoção bloqueada" : "Remover Magalu"}
      >
        <X className="h-4 w-4" />
      </Button>
    </div>
...
// ================================
const MagaluMarketplaceSection: React.FC<MagaluMarketplaceSectionProps> = ({
  marketplaces = [],
  setMarketplaces,
  loading,
  ...
  const removerItem = (idx: number) => {
    ...
    setMarketplaces((prev: MagaluMarketplaceItem[]) =>
    ...
    <div className="space-y-2 overflow-visible">
      {bloquearEdicao && (
        <div className="rounded-xl border border-amber-400/20 bg-amber-400/10 px-4 py-3 text-xs font-medium text-amber-100/80">
          Marketplace Magalu bloqueado para edição. Edite apenas os percentuais
          no cálculo de preço.
        </div>
      )}

      {marketplacesSeguros.map((item: MagaluMarketplaceItem, idx: number) => (
        <MagaluMarketplaceItemRow
          key={`${item.marketplace || "magalu"}-${idx}`}
          item={item}
          idx={idx}
...
export default MagaluMarketplaceSection;