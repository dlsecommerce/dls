  ...
  return (
    <div className="w-full">
      ...
      <Select value={value || "Todos"} onValueChange={onChange}>
        <SelectTrigger className="h-11 w-full cursor-pointer rounded-lg border border-neutral-700 bg-[#161616] px-4 text-left text-sm text-white outline-none transition focus:ring-2 focus:ring-green-500/20">
          <SelectValue placeholder={placeholder} />
        </SelectTrigger>

        <SelectContent className="rounded-lg border border-neutral-700 bg-[#1b1b1b] text-white shadow-xl">
          {options.map((option) => (
...
export default function TrayFiltersSidebar({
  ...
  const updateFilter = (key: keyof TrayFilters, value: string) => {
    setFilters((prev) => ({
      ...prev,
    ...
    <div className="w-full bg-transparent px-4 py-4">
      <div className="w-full max-w-[220px] space-y-5">
        ...
        <FilterSelectBlock
          ...
          value={filters.lojasVirtuais}
          onChange={(v) => updateFilter("lojasVirtuais", v)}
