...
export function buildOrSearchParts(tokens: string[]) {
  ...
  for (const term of tokens) {
    ...
    for (const v of variants) {
      ...
      orParts.push(`code.ilike.${pattern}`);
      orParts.push(`mark.ilike.${pattern}`);
      orParts.push(`product.ilike.${pattern}`);
...
// --- Usadas em Costadjustments.tsx e Costdatatable.tsx ---
...
export function sanitizeDecimalInput(value: string): string {
  ...
  if (firstSeparatorIndex !== -1) {
    const before = cleaned.slice(0, firstSeparatorIndex + 1);
...
export function formatDecimalOnBlur(value: string): string {
...
export function parseDecimalToNumber(value: string): number | null {
