...
export default function ConfirmImportModalMarketplace({
  open,
  onOpenChange,
  count,
  onConfirm,
  ...
  const titulo =
    customTitle ?? (isInclusao ? "Confirmar Inclusão de Marketplaces" : "Confirmar Alteração de Marketplaces");

  const texto =
    customText ??
    (isInclusao
      ? "Você está prestes a INCLUIR novos registros de marketplace no sistema."
      : "Você está prestes a ALTERAR registros de marketplace existentes.");
